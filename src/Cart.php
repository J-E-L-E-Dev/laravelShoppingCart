...
class Cart
{
    ...
    public function total()
    {
        ...
        switch (config('cart.driver')) {
            case 'GENERAL':
            case 'PNP':
                return $this->generalTotal($content);
                break;
            
            case 'HKA':
                return $this->hkaTotal($content);
                break;
    ...
    protected function hkaTotal($content)
    {
        ...
        foreach ($content as $key => $value) {
            $base += $this->hka($value->qty, $value->price);
        }

        $taxes = $this->tax();
        $total_taxes = $taxes[config('cart.taxes.0.name')]['value'] + $taxes[config('cart.taxes.1.name')]['value'] + $taxes[config('cart.taxes.2.name')]['value'] + $taxes[config('cart.taxes.3.name')]['value'];

        $totalCost = $this->extraCosts->reduce(function ($total, $cost) {
            return $total + $cost;
        }, 0);

        $total = $totalCost + $base + $total_taxes;
    ...
    public function tax()
    {
        $content = $this->getContent();
        $taxes = [
            config('cart.taxes.0.name') => [
                'IVA' => config('cart.taxes.0.value'),
                'value' => 0
            ],
            config('cart.taxes.1.name') => [
                'IVA' => config('cart.taxes.1.value'),
                'value' => 0
            ],
            config('cart.taxes.2.name') => [
                'IVA' => config('cart.taxes.2.value'),
                'value' => 0
            ],
            config('cart.taxes.3.name') => [
                'IVA' => config('cart.taxes.3.value'),
                'value' => 0
            ]
        ];

        return $this->totalTaxes($content, $taxes);
    }
    ...
    protected function generalDriver($input, array $taxes)
    {
        ...
        foreach ($input as $key => $value) {
                ...
                $taxes[config('cart.taxes.0.name')]['value'] += $this->numberFormat($value->qty * $value->tax);
    ...
    protected function hkaDriver($input, array $taxes)
    {
        ...
        foreach ($input as $key => $value) {
            if ($value->aliquot == 0) {
                $general += $this->hka($value->qty, $value->price);
            } elseif ($value->aliquot == 1) {
                $exempt += $this->hka($value->qty, $value->price);
            } elseif ($value->aliquot == 2) {
                $reduced += $this->hka($value->qty, $value->price);
            } elseif ($value->aliquot == 3) {
                $luxury += $this->hka($value->qty, $value->price);
            }
        }

        $taxes[config('cart.taxes.0.name')]['value'] += CartItem::calculateTaxes($general, config('cart.taxes.0.value'));
        $taxes[config('cart.taxes.1.name')]['value'] += CartItem::calculateTaxes($exempt, config('cart.taxes.1.value'));
        $taxes[config('cart.taxes.2.name')]['value'] += CartItem::calculateTaxes($reduced, config('cart.taxes.2.value'));
        $taxes[config('cart.taxes.3.name')]['value'] += CartItem::calculateTaxes($luxury, config('cart.taxes.3.value'));
    ...
    protected function pnpDriver($input, array $taxes)
    {
        ...
        foreach ($input as $key => $value) {
            if ($value->aliquot == 0) {
                $general += $this->pnp($value->qty, $value->price);
            } elseif ($value->aliquot == 1) {
                $exempt += $this->pnp($value->qty, $value->price);
            } elseif ($value->aliquot == 2) {
                $reduced += $this->pnp($value->qty, $value->price);
            } elseif ($value->aliquot == 3) {
                $luxury += $this->pnp($value->qty, $value->price);
            }
        }
    ...
    {
        return intval(($qty * $value) * 100) / 100;
    }
    ...
    public function merge($identifier, $dispatchAdd = true, $instance = self::DEFAULT_INSTANCE)
    {
        ...
        foreach ($storedContent as $cartItem) {
            $this->addCartItem($cartItem, $dispatchAdd);
        }
     ...
     * Add an item to the cart.
     *
     * @param \JeleDev\Shoppingcart\CartItem $item          Item to add to the Cart
     * @param bool                              $keepDiscount  Keep the discount rate of the Item
     * @param bool                              $keepTax       Keep the Tax rate of the Item
     * @param bool                              $dispatchEvent
     *
     * @return \JeleDev\Shoppingcart\CartItem The CartItem
     */
